		...
		<div class="content-wrapper">
			<div class="container-xxl flex-grow-1 container-p-y">
				...
				<div class="card">
					...
					<form action="<c:url value='/admin_loc_duandabaocao_theoquynam_post?action=loc_duandabaocao_theoquynam_post'/>" method="post">
					  <div class="card-body">
                    ...
                    <div class="col-md-2">
                      ...
                      <select class="form-select placement-dropdown" id="selectYear" name="thuocNam">
						  ...
						  <div class="d-flex flex-row">
		                     ...
		                     <a  style="margin-left:10px;" href='<c:url value="/admin_quanlyduan_dabaocao?action=danhsach_duan_dabaocao&page=1&maxPageItem=40&sapxeptheothuoctinh=thuocnam&sapxeptheochieu=desc"/>' class="btn btn-info">Làm mới dữ liệu</a>
	                            <button onclick="tableToExcel()" style="float:right;margin-left:10px;" class="btn btn-primary ">Xuất file Excel</button>
                  ...
                  <form action="<c:url value='/admin_timkiem_duandabaocao_theoquynam_post?action=timkiem_duandabaocao_theoquynam_post'/>"  method="post">
					  <div class="card-body">
                              <div class="row gx-3 gy-2 align-items-center">
                ...
                <form action="<c:url value='/admin_quanlyduan_dabaocao?action=danhsach_duan_dabaocao'/>"
								...
								<c:forEach var="item" items="${duandabaocao.danhsach}">
									<tr>
									  <td ><%=i++ %></td>
										...
										<td class="post_tongvon_thuchien">${item.tongVonThucHien} ${item.tenDonViTienTe}</td>
             						    <td class="post_vontaitro">${item.vonTaiTro} ${item.tenDonViTienTe}</td>	
             						    <td class="post_vonvay">${item.vonVay} ${item.tenDonViTienTe}</td>	
             						    <td class="post_vonkhac">${item.vonKhac} ${item.tenDonViTienTe}</td>	
             						    <td class="post_vondoiung">${item.vonDoiUng} ${item.tenDonViTienTe}</td>	
                              ...
                              > 
                               <a
														class="dropdown-item"
														   ...
														   href='<c:url value="/admin_xoa_duan_dabaocao?maDuAn_DaThamGia=${item.maDuAn_DaThamGia}&action=xoa_duan_dabaocao"/>'
				...
				$(function() {
					window.pagObj = $('#pagination').twbsPagination({
						...
						onPageClick : function(event, page) {
							if(currentPage!=page)
								{
								$('#action').val('danhsach_duan_dabaocao');
