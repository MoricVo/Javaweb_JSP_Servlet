...
public class duan_dabaocaoDAO implements DAOInterface<bang_duan_dathamgia_model> {
	...
	public int InsertAll(String[] thuoctinh) {
		...
		try {
			...
			String sql = "INSERT INTO bang_duanquocte_dabaocao (maduan_dabaocao,maduan,chuduan,tendonvitructhuoc,tendanhmuc,tennhataitro,tenloaivientro,tencoquanpheduyet,tendoitacthuchien,tenduantiengviet,tenduantienganh,thanhvienthamgia,muctieuduan,noidungthuchien,thietbitaitro,kinhphinamtruoc,tongvonthuchien,vontaitro,vonvay,vonkhac,vondoiung,tendonvitiente,thoigianthuchien,thoigianbatdau,thoigianketthuc,tiendothuchien,phantramtiendo,thuocquy,thuocnam) "
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			PreparedStatement st = con.prepareStatement(sql);
	        ...
	        for (String Cacthuoctinh : thuoctinh) {
	        	  ...
	        	  String thuocQuy = ids[26];
	        	  String thuocNam = ids[27];
