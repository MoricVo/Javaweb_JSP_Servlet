...
public class duanDAO implements DAOInterface<bang_duan_model>
{
	...
	public int insert(bang_duan_model t) {
		...
		try {
			...
			String sql = "INSERT INTO bang_duanquocte (maduan,manguoidung,madonvitructhuoc,madanhmuc,"
					...
					+ "vontaitro,vonvay,vonkhac,vondoiung,madonvitiente,thoigianthuchien,thoigianbatdau,thoigianketthuc,tiendothuchien,phantramtiendo,ngaytao)"
					+ " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int update(bang_duan_model t) {
		...
		try {
			...
			",tiendothuchien=?"+",phantramtiendo=?"+",ngaycapnhat=?"   + " WHERE maduan=?";
	...
	public int update_trangthai_3(String[] madonvitructhuoc) {
		...
		public ArrayList<bang_duan_model> selectVonLon(int manguoidung) {
			...
			try {
				...
				// Bước 2: tạo ra đối tượng statement
				String sql = "SELECT bang_duanquocte.tongvonthuchien,bang_duanquocte.tenduan_tiengviet,bang_nhataitro.tennhataitro,bang_donvitiente.tendonvitiente\r\n" + 
						"FROM bang_duanquocte\r\n" + 
					 ...
					 String tenDuAn_TiengViet=rs.getString("tenduan_tiengviet");
					  ...
					  bang_nhataitro_model ntt=new bang_nhataitro_model();
						 ntt.setTenNhaTaiTro(rs.getString("tennhataitro"));
							bang_duan_model kh = new bang_duan_model(tenDuAn_TiengViet,tongVonThucHien,tt,ntt);
							ketQua.add(kh);
				}
		...
		public ArrayList<bang_duan_model> selectVonLonAll() {
			...
			try {
				...
				// Bước 2: tạo ra đối tượng statement
				String sql = "SELECT bang_duanquocte.tongvonthuchien,bang_duanquocte.tenduan_tiengviet,bang_nhataitro.tennhataitro,bang_donvitiente.tendonvitiente,bang_donvitructhuoc.tendonvitructhuoc\r\n" + 
						"FROM bang_duanquocte\r\n" + 
						"LEFT JOIN bang_nhataitro ON bang_duanquocte.manhataitro = bang_nhataitro.manhataitro\r\n" + 
						"LEFT JOIN bang_donvitiente ON bang_duanquocte.madonvitiente=bang_donvitiente.madonvitiente \r\n" + 
				...
				while (rs.next()) {
					 ...
					 String tenDuAn_TiengViet=rs.getString("tenduan_tiengviet");
					  ...
					  bang_nhataitro_model ntt=new bang_nhataitro_model();
						 ntt.setTenNhaTaiTro(rs.getString("tennhataitro"));
							bang_duan_model kh = new bang_duan_model(tenDuAn_TiengViet,tongVonThucHien,donvitructhuoc,tt,ntt);
							ketQua.add(kh);
				}
